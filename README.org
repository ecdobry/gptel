...
***** With automatic model discovery (Recommended)

llama.cpp's [[https://github.com/ggml-org/llama.cpp/blob/master/tools/server/README.md#api-endpoints][router mode]] supports multiple models and exposes a =/models= endpoint.  gptel can fetch available models automatically:

#+begin_src emacs-lisp
(require 'gptel-llama-cpp)

(gptel-make-llama-cpp "llama-cpp"       ;Any name
  :host "localhost:8080"                ;Server location with port
  :stream t)                            ;Stream responses
#+end_src

Models are fetched automatically when the backend is created.  To refresh the model list later (e.g., after loading new models on the server), use =M-x gptel-llama-cpp-refresh-models=.

***** With manual model specification

If you prefer to specify models manually, or are using a single-model server:
...
(gptel-make-llama-cpp "llama-cpp"
  :host "localhost:8080"
  :stream t
  :models '(llama-3.2-3b-instruct       ;Simple model name
            (llava-1.6                  ;Model with capabilities
             :description "Vision model"
             :capabilities (media)
             :mime-types ("image/jpeg" "image/png"))))
#+end_src

***** Using OpenAI-compatible API (Alternative)

You can also use =gptel-make-openai= directly since llama.cpp offers an OpenAI-compatible API:

#+begin_src emacs-lisp
(gptel-make-openai "llama-cpp"          ;Any name
  :stream t                             ;Stream responses
  :protocol "http"
  :host "localhost:8080"                ;Server location
  :models '(my-model))                  ;Model names (any names work)
#+end_src
...
;; OPTIONAL configuration
(setq
 gptel-model   'llama-3.2-3b-instruct
 gptel-backend (gptel-make-llama-cpp "llama-cpp"
                 :host "localhost:8080"
                 :stream t))
#+end_src
